...
function anomaly_score(advae::adVAE, real_input; L=100, dims=3, batch_size=64)
...
function StatsBase.fit!(advae::adVAE, data, params)
	...
	return history, best_adVAE, sum(map(p->length(p), ps_step1))+sum(map(p->length(p), ps_step2)), iters
end
