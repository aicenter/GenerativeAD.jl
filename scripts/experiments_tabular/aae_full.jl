...
function fit(data, parameters)
	...
	model = GenerativeAD.Models.aae_constructor(;idim=size(data[1][1],1), 
	...
	training_info, 
		...
		(x -> sample_reconstruction_batched(info.model, x, L, 512), merge(parameters, (score = "reconstruction-sampled", L=L)))
			...
			[(x -> aae_score_batched(model, x, alpha, 512), 
			merge(parameters, (alpha = alpha, score = "disc"))) 
